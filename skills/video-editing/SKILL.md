...
This skill is bundled in the Video Maker Agent Plugin for ChatGPT desktop Work and local Codex. The plugin root is two directories above this skill folder. Call `workspace_info` for the actual asset/project workspace; the installed plugin defaults to `~/Movies/VideoMaker`, configurable through `VIDEO_MAKER_WORKSPACE`. Never store user media in the versioned plugin cache. The development CLI defaults to `<repository>/workspace`.

Use the video-maker MCP if connected. If only a remote/cloud Work environment is available, report that local MCP tools require ChatGPT desktop; do not claim you can reach the Mac's files from the cloud. When a local shell and the installed runtime are available, the CLI can run the same pipeline from the plugin/repository root:
...
1. Discover actual assets with `list_assets`, or copy user-selected files using `import_asset`. Inspect durations and sampled frames with `inspect_media` / `inspect_frames` before selecting cuts. A filename alone does not establish what a recording shows. Request a relevant timestamp or inspect more frames if necessary. `detect_silence` proposes ranges; a silent screen recording may still contain important actions.
...
4. Save the strict project JSON. Use relative paths returned by tools, stable scene IDs, and one narration/character configuration per scene. Different scenes may have different characters. Set `source_in`, `speed`, `crop`, and output `duration` to select recording segments. Short source clips hold the last frame; inspect whether that looks intentional. Narration determines scene duration when `duration` is omitted.
5. Put relevant source/character credits in `credits`; this creates a text sidecar, not a visible credit scene. Add a final title scene when an on-screen credit is wanted. Respect the supplied asset readme and actual character terms.
6. `render_preview` returns immediately with a job ID. Poll `job_status` at sensible intervals until complete/failed. Open the resulting video and inspect frames at cuts and caption changes; check that important UI is not obscured. Revise the project using `save_project(overwrite=true)`; it keeps the previous JSON in `revisions/`.
7. Use `render_final` for full resolution. Deliver the MP4 plus the editable project and SRT. State any actual remaining limitation. Failed jobs return a concrete error; fix it before reporting success.
