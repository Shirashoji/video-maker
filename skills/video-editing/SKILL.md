...
## Code-authored explanations, HTML slides and motion graphics

Author diagrams and animated slides in `scene.graphics`. In addition to primitive shapes (`text`, `rect`, `ellipse`, `arrow`, `line`, `image`), you can use **`kind: "html"`** (rendered via Headless Chrome), **`kind: "mermaid"`** (rendered via Mermaid CLI), and **`kind: "svg"`** (rendered via resvg). This avoids fragile manual coordinate calculations by leveraging Web standards (Flexbox/Grid) and automatic graph layout. Consult `docs/project-format.md` and `examples/html-mermaid-showcase.json`.

Use `asset_template` (or CLI `asset-template`) to get responsive 16:9 HTML slide starters and Mermaid diagrams. Before adding them to a project, call `render_asset(source)` (or CLI `render-asset`) to immediately rasterize a transparent PNG and inspect it with `inspect_media` to verify layout and text wrap. `enter`/`exit` provide fade, slide and pop effects, and `keyframes` control position, scale and opacity on all elements.
